...
class ImpactorSimulatorGUI:
    ...
    def update_plot(self):
        ...
        self.ax.plot(self.test_time, self.test_acceleration, label="Deformation Acceleration")
    ...
    def impactor_dropped(self, event):
        ...
        [self.test_time, self.test_acceleration] = self.simulator.calculations.read_csv()
        print(self.test_time)
        print(self.test_acceleration)

        self.update_display()

        self.simulator.acceleration_filtered = self.simulator.calculations.bandpass_filter(self.test_acceleration, 0.001, 2, 20000, 2) # (data, lowcut, highcut, fs, order), simulation (data, 0.001, 2, 20000, 2)
        print(self.simulator.acceleration_filtered)

        self.update_plot()

    def export_data(self):
        file_path = os.path.join(os.path.dirname(__file__), '..', 'output_csv', "output_data_simulation.csv")
